...
public class opcionAve {
	...
	private void initialize() {
		frame = new JFrame();
		frame.setTitle("Ventana para registrar un Ave");
		...
		JLabel ventanam = new JLabel("Registro de Aves");
...
//METODO PARA AÑADIR LO INGRESADO A LA BASE DE DATOS
	private void IngresarActionPerformed(java.awt.event.ActionEvent evnt) {
			Aves ave = new Aves();
			ave.setNombre(Nombre.getText());
			ave.setGenero(Genero.getText());
			ave.setEdad(Integer.parseInt(Edad.getText()));
			try {
				Object[] objeto = {ave.getNombre(),ave.getGenero(),ave.getEdad()};
				DB database = DB.getInstances();
				database.dbPrepareStatement(objeto,"INSERT INTO ave (NombreAnimal,Genero,Edad)"+"VALUES (?,?,?)" );
			}
			finally {
