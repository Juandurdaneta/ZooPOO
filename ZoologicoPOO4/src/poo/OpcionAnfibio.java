...
public class OpcionAnfibio {
	...
	private void initialize() {
		frame = new JFrame();
		frame.setTitle("Ventana para registrar un Anfibio");
		...
		JLabel ventanam = new JLabel("Registro de Anfibios");
		...
		JLabel edad = new JLabel("Edad del Anfibio");
	...
	//METODO PARA AÑADIR LO INGRESADO A LA BASE DE DATOS
	private void IngresarActionPerformed(java.awt.event.ActionEvent evnt) {
		Anfibio anfibio = new Anfibio();
		anfibio.setNombre(Nombre.getText());
		anfibio.setGenero(Genero.getText());
		anfibio.setEdad(Integer.parseInt(Edad.getText()));
		try {
			Object[] objeto = {anfibio.getNombre(),anfibio.getGenero(),anfibio.getEdad()};
			DB database = DB.getInstances();
			database.dbPrepareStatement(objeto,"INSERT INTO anfibio (NombreAnimal,Genero,Edad)"+"VALUES (?,?,?)" );
		}
		finally {
